...
export const RoleRoutes = () => {
    router.group(() => {
       router.post('/', [RolesController, 'store']).use([middleware.auth({ guards: ['api'] }), middleware.checkPermission(['create-role'])]),
        ...
        router.get('/', [RolesController, 'getRoles']).use([middleware.auth({ guards: ['api'] }), middleware.checkPermission(['view-roles'])])
    }).prefix('/roles')
}